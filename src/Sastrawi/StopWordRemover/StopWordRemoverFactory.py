...
class StopWordRemoverFactory(object):
    """Factory for StopwordRemover"""

    __DEFAULT_STOP_WORDS = [
    ...
    def __init__(self):
        self._stop_words = self.__DEFAULT_STOP_WORDS
    ...
    def stopwords(self, new_stopwords):
        self._stop_words = new_stopwords
    ...
    @classmethod
    def from_file(cls, filepath, delimiter=",", **kwargs):
        """Create StopWordRemoverFactory instance with stopwords from file
        ...
        :param delimiter: delimiter for each stopwords
        :param replace: if set to True, will replace the default stopwords.
        :return: class instance
        :rtype: StopWordRemoverFactory
        """
        ...
        with open(filepath, 'rb') as f:
            ...
            for r in reader:
                _stack += r
        _stack = list(set(_stack))

        instance = cls()
        if replace:
            instance.stopwords = _stack
        else:
            instance.append_stop_words(_stack)

        return instance

    def append_stop_words(self, stopwords):
        """Append stopwords to default stopwords

        :param stopwords: stopwords to append
        :type stopwords: list
        """

        if not isinstance(stopwords, list):
            raise TypeError("expecting list, {} found".format(type(stopwords)))

        self._stop_words = list(set(self.__DEFAULT_STOP_WORDS + stopwords))
